...
create table public.users (
  id uuid not null primary key references auth.user on delete cascade,
  email text
);
...
create table public.player (
  id bigint generated by default as identity primary key,
  name text not null,
  clan_id bigint references public.clan on delete cascade not null,
...
create table chest (
  ...
  source text,
  level int,
  player_id bigint references public.player on delete cascade not null,
  chest_type_id bigint references public.chest_type on delete cascade not null,
...
BEGIN
    RETURN QUERY
    SELECT
        p.id::integer AS player_id, -- Explicitly cast to integer
...
--Function to assign chests from 1 player to other
CREATE OR REPLACE FUNCTION change_chest_player_id(oldPlayerId bigint, newPlayerId bigint)
RETURNS void AS $$
BEGIN
-- Update player_id in chest table
    UPDATE chest
    SET player_id = newPlayerId
    WHERE player_id = oldPlayerId;

    -- Delete oldPlayerId from player table
        DELETE FROM player
